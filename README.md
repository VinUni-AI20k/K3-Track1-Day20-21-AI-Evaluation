...
Phase 0: Re-Audit & Integration ────► Baseline (44/44 tests pass) & Corpus Audit (18 docs, 341 sections)
       │
Phase 1: Intentional Coverage ──────► Dimensions (D1–D4), Values (V01–V15), Combinations (C01–C15) [Checkpoint A/B Locked]
       │
Phase 2: Human Baseline ────────────► Independent Labels (Huy, Huế), Inter-Annotator Agreement, Consensus Gold
       │
Phase 3: Observable Rubric & Routing ► Deterministic Code Checks vs Semantic LLM Judge vs Human Policy
       │
Phase 4: Code Checks & Calibration ─► 6 Code Checks (15/15 tests pass), LLM Judge Multi-Criterion Harness
       │
Phase 5: Locked Thresholds & Slices ─► Thresholds locked BEFORE candidate run, Slice Scorecard by D1–D4
       │
Phase 6: Release Verdict & Report ──► PM Verdict (Ship/Hold), 5-Part Executive Summary, Full REPORT.md
```
...
| **Phân luồng Routing** | [`deliverables/evidence/routing-table.md`](deliverables/evidence/routing-table.md) | Ma trận phân bổ tiêu chí vào Code / Judge / Human | Specified |
| **Ngưỡng khóa** | [`deliverables/evidence/thresholds-locked.md`](deliverables/evidence/thresholds-locked.md) | Ngưỡng chất lượng khóa trước khi chạy candidate | Locked |
...
# b. 15 unit tests cho hệ thống Code Checks mở rộng
...
2. **Code Checks First**: Bất kỳ tiêu chí nào đo được bằng cú pháp, schema, hay đối chiếu chuỗi (`citation_exists`, `quote_verbatim`, `scope_sources_consistency`) phải đẩy về Code Check để đạt độ trễ ~0ms, chi phí $0 và 100% deterministic.
