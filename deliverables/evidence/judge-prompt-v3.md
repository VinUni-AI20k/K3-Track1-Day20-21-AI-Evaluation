# Judge prompt v3 — GROUNDEDNESS + SCOPE của AI Tutor
<!-- v3: ĐỔI ĐÚNG MỘT THỨ so với v2 — viết lại tiêu chí 3.
     Lý do: v1+v2 chặn nhầm 7 ca vì phạt mọi khẳng định không nằm NGUYÊN VĂN trong
     quote. Nhưng judge chỉ được đưa đoạn quote ngắn, KHÔNG được đưa toàn văn section
     (xem build_judge_prompt trong eval/judge.py) — nên 'không có trong quote' không
     chứng minh được là bịa. Ca điển hình sc-13: số 92% có thật trong section
     ai-evals-m03#evaluation-rubric đã cite, chỉ không nằm trong đoạn quote. -->
...
3. `answer` khẳng định một dữ kiện MỚI mâu thuẫn với `quote`, hoặc dựng lên một khái
   niệm/thực thể không hề xuất hiện trong `quote` nào.
   **Lưu ý quan trọng:** bạn chỉ được xem một đoạn `quote` NGẮN trích từ section, không
   phải toàn văn section. Vì vậy "câu này không có chữ nào trong quote" KHÔNG đủ để kết
   luận là bịa. Chỉ chấm FAIL khi answer **mâu thuẫn** với quote, hoặc khi con số được
   trình bày như một **ngưỡng/khuyến nghị lấy từ tài liệu** mà quote lại ghi con số khác.
   Diễn giải bằng lời khác, tóm tắt, và ví dụ minh hoạ mở đầu bằng "ví dụ", "chẳng hạn"
   là việc bình thường của trợ giảng — **PASS**.
